...
HC595 *_hc595 = NULL;
HC595_Status_t HC595_AssignPin(HC595 *hc595, GPIO_TypeDef *port, uint16_t pin, HC595_PinName HC595_PinName) {
	if (!hc595) return HC595_INVALID_ARG;
	switch (HC595_PinName) {
	case HC595_CLK:
		hc595->clk.port = port;
		hc595->clk.pin = pin;
		break;
...
void HC595_ReturnValueToString(char *s, HC595_Status_t retVal) {
	...
	case HC595_OK:
		strcpy(s, "HC595_OK");
		break;
	case HC595_BEYOND_MAX_CASCADE:
		strcpy(s, "HC595_BEYOND_MAX_CASCADE");
		break;
	case HC595_INVALID_ARG:
		strcpy(s, "HC595_INVALID_ARG");
		break;
	case HC595_ERROR:
		strcpy(s, "HC595_ERROR");
		break;
	}
}
...
void HC595_TestPin(HC595_PinName HC595_PinName) {
	while (!_hc595);
	HC595_WRITE(HC595_PinName, 1);
	DELAY_MS(2000);
	HC595_WRITE(HC595_PinName, 0);
	DELAY_MS(2000);
}
